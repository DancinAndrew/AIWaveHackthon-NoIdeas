...
- 廠商回報訂單成立後，於原對話與「我的預約」揭露該筆訂單的預計 OPENPOINT 回饋點數、計算依據與發放條件；狀態沿用 `mms_order_record.point_status` 的 `01 待發放`。
- 廠商回報完工後，住戶在原對話明確驗收才結案；點數依完工金額重算後轉為 `02 已發放`，並寫入 append-only 流水帳。廠商單方回報完工不得結案，重複驗收不得重複入帳。
...
- 不對 OPENPOINT 正式會員帳戶發放、折抵或收回點數。回饋只在平台內記帳並向住戶明示為 Demo，實際發放、`used_points` 折抵與退點不在 MVP 範圍（見 [`ADR-0007`](docs/adr/0007-disclose-openpoint-rewards-as-platform-demo-ledger.md)）。
