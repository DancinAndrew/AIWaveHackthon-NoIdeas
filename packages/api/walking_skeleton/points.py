...
# 商品訂單的基礎金額由伺服器從目錄算出，不接受廠商回報，也不需要類別估算。
AMOUNT_SOURCE_ORDER_FINAL = "order_final_amount"

_AMOUNT_SOURCE_LABELS = {
    AMOUNT_SOURCE_PROVIDER: "廠商回報預估金額",
    AMOUNT_SOURCE_BASELINE: "平台類別估算金額",
    AMOUNT_SOURCE_ORDER_FINAL: "訂單實付金額",
}
...
def estimate_reward(
    *,
    ...
    基礎金額與來源由呼叫的 flow 決定：維修在技師報價前沒有價格，所以用廠商回報值或
    類別基準；商品訂單已有伺服器算出的實付金額，估算反而會憑空造出一個平台其實已經
    知道的數字。這裡只負責費率、上限與揭露欄位。
    """
