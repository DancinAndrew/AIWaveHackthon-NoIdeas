...
class ProductPurchaseFlow:
    ...
    def start(
        ...
        if prompt is None:
            return self._present_candidates(svc, conversation, request, opening=True)
    ...
    def continue_turn(
        ...
        if stage in {"collecting_details", "awaiting_resident_selection", "out_of_stock"}:
            ...
            return self._present_candidates(svc, conversation, request)

        if stage == "awaiting_resident_confirmation":
            if any(phrase in content for phrase in CONFIRM_PHRASES):
                return self._confirm_and_dispatch(svc, conversation, request)
        ...
        if stage == "authorizing_payment":
            return self._confirm_and_dispatch(svc, conversation, request, retry=True)
    ...
    def _confirm_and_dispatch(
        ...
        *,
        ...
        if not self.authorize_payment(self._order_snapshot(request)):
            ...
            return svc.turn_payload(conversation, assistant, artifact=artifact)
